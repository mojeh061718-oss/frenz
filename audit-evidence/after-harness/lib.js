...
const BACKUP = process.env.LIVE ? ROOT : path.join(ROOT, 'backup/v10.23');
...
function load() {
  ...
  vm.runInContext(fs.readFileSync(path.join(BACKUP, 'js/personas.js'), 'utf8'), ctx, { filename: 'js/personas.js' });
  vm.runInContext(fs.readFileSync(path.join(BACKUP, 'js/api.js'), 'utf8'), ctx, { filename: 'js/api.js' });
